...
(function() {
  ...
  function tryPopulateDEMEntry(tileID) {
    ...
    const requested = tileID && tileID.canonical;
    const provided = demTile.tileID && demTile.tileID.canonical;
    if (!requested || !provided) return;

    if (requested.z !== provided.z || requested.x !== provided.x || requested.y !== provided.y) {
      return;
    }
  ...
  const terrainNormalLayer = {
    ...
    ensureTextureForTile(tileID) {
      ...
      if (demEntry.tileID && demEntry.tileID.key && demEntry.tileID.key !== tileID.key) {
    ...
    render(gl, options) {
      ...
      const seen = new Set();
      for (const tileID of tileIDs) {
        const key = tileID && tileID.key;
        if (!key || seen.has(key)) continue;
        seen.add(key);
        uniqueTiles.push(tileID);
      }
      const renderableTiles = uniqueTiles;
      ...
      if (currentMode === "snow" || currentMode === "slope") {
        ...
        this.renderTiles(gl, shader, renderableTiles, options);
